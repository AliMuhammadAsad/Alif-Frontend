...
class RegexTokenizerFast(Tokenizer):
    ...
    def presplit(self, text, tqdm_bar=None):
        ...
        j = 0
        # slow as well:
        print(f"\t Adding padding to chunks now")
        for chunk in text_chunks:
            try:
                chunk_length = len(chunk.encode("utf-8"))
                #print(f"Chunk {chunk} of length {chunk_length}")
    ...
    def train(self, text, vocab_size, model_prefix, verbose=False, tqdm_bar=None):
        print(f"In RegexFast - train method")
        ...
        if tqdm_bar:
            tqdm_bar.update(1)
        print(f"In RegexFast - Starting training...")
        self.merges, self.vocab = trainFast(id_list, vocab_size, self.init_tokens)
        print(f"In RegexFast - Training complete!")
        if tqdm_bar:
            tqdm_bar.update(1)
